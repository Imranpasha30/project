...
def get_current_location():
    try:
        ...
        if ($loc.Permission -eq 'Granted') {
            ...
            $loc.Stop()
            $position | Select-Object Latitude, Longitude, Altitude | ConvertTo-Json
        } else {
        ...
        location_data = json.loads(process.stdout)
...
async def send_message(token, chat_id, frame, location=None):
    try:
        now = datetime.now()
        formatted_now = now.strftime(" DATE: %d/%m/%Y, TIME: %I:%M:%S %p")
